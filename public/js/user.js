...
function rcShowDay(ds){
  ...
  if(roomInfo&&roomInfo.status!=='available'&&isRoomBlocked(rcRoom,ds)){
    const lbl={maintenance:'ðŸ”§ Sedang Maintenance',closed:'ðŸš« Ruangan Ditutup'};
  ...
  if(!list.length){
    listEl.innerHTML=`<div style="font-size:12.5px;color:var(--green);padding:9px 12px;background:var(--green-bg);border-radius:8px;border:1px solid rgba(26,127,75,.2)">âœ“ Tidak ada reservasi — ruangan tersedia.</div>`;
    det.style.display='block';return;
  }
  ...
  <div class="form-card" id="booking-form">
    <div class="fc-inner">
      ...
      <div class="fc-section">
        ...
        <!-- TIME -->
        <div class="fc-field">
          <label class="fc-label">Rentang Jam Pemakaian <span class="req">*</span> <span style="font-size:11px;color:var(--text3);font-weight:400">(07:00 – 17:00 WIB)</span></label>
          <div class="time-row">
...
function renderUserCalDays(selectedDate){
  ...
  for(let i=0;i<firstDay;i++)html+=`<div class="ucal-day ucal-empty"></div>`;
  for(let d=1;d<=daysInMonth;d++){
    ...
    if(isToday&&!isSel)cls+=' ucal-today';
    ...
    if(data.success) {
      ...
      document.getElementById('booking-form').innerHTML=`
        <div class="success-state">
          ...
          <span class="sbadge pending">â— Menunggu Konfirmasi Admin</span>
...
/* â”€â”€â”€ SCROLL ANIMATION OBSERVER â”€â”€â”€ */
...
async function init(){
  ...
  await showTyping();
  addBotMsg(`Selamat datang di <strong>Sistem Pemesanan Ruangan Laboratorium Terpadu</strong> Universitas Tanjungpura. ðŸ‘‹<br>Saya adalah <strong>LabBot</strong>, asisten pemesanan digital Anda.`);
  await showTyping();
