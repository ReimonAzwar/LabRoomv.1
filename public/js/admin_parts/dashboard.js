...
function buildRows(data) {
    ...
    return data.map(b => {
        ...
        return `
            <div class="brow">
                <div class="c">
                    <div class="cn">${escapeHTML(b.nama)}${conflictBadge}</div>
                    <div class="cs">${escapeHTML(b.kontak)}</div>
                </div>
                <div class="c col-inst" style="font-size:12.5px;color:var(--text2)">${escapeHTML(b.instansi)}</div>
                <div class="c" style="font-size:12.5px">${escapeHTML(b.ruangan)}</div>
                <div class="c col-time">
                    ${typeof formatDate === 'function' ? formatDate(b.tanggal) : b.tanggal}<br>
                    <span style="color:var(--text3);font-size:11.5px">${escapeHTML(b.jamMulai)}–${escapeHTML(b.jamSelesai)}</span>
                </div>
                <div class="c">
